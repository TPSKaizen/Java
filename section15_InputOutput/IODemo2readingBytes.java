...
public class IODemo2readingBytes {
		...
		public static void fileCopyNoBuffer() {
			...
			try (FileInputStream in = new FileInputStream(inFileStr);
					FileOutputStream out = new FileOutputStream(outFileStr)) { //tryWithResources syntax
				...
				while ((byteRead = in.read()) != -1) {
					...
					// bytes
					out.write(byteRead); //writes one byte of at a time
				}
		...
		public static void fileCopyWithBufferAndArray() {
			...
			long startTime, elapsedTime; // for speed benchmarking
			startTime = System.nanoTime();
			try (BufferedInputStream in = new BufferedInputStream(new FileInputStream(inFileStr)); //tryWithResources and Decorator pattern
